...
function OutputFieldPills({ nodeId }: { nodeId: string }) {
  const editorNodes = useImovsStore((s) => s.editorNodes);
...
function ImovsNodeComponent({ id, data, selected }: NodeProps) {
  const { selectedNodeId, setSelectedNodeId, deleteNode } = useImovsStore();
...
export const ImovsNode = memo(ImovsNodeComponent);

export const nodeTypes = {
  manualTrigger: ImovsNode,
  webhookTrigger: ImovsNode,
  scheduleTrigger: ImovsNode,
  httpRequest: ImovsNode,
  code: ImovsNode,
  if: ImovsNode,
  set: ImovsNode,
  merge: ImovsNode,
